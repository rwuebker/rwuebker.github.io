...
import { routeUserInput } from "@/lib/research/router";
import { executeAction, getLastReport, askQuestion } from "@/lib/research/actions";
import {
...
import { RESEARCH_API_BASE } from "@/lib/research/config";
import type { AnalysisContext, AskResponse, ResearchReport, ProjectBlockState, ResearchProjectGuidance, ResearchProjectState } from "@/lib/research/types";
...
async function downloadNotebook(msg: Message) {
  try {
    ...
    if (runId) {
      const byRunRes = await fetch(`${RESEARCH_API_BASE}/analyze/notebook`, {
        method: "POST",
      ...
      if (byRunRes.ok) {
        ...
        if (data?.notebook) {
          ...
          a.href = url;
          a.download = "research_analysis.ipynb";
          a.click();
    ...
    const res = await fetch(`${RESEARCH_API_BASE}/analyze/notebook`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function downloadPdfReport(msg: Message) {
  try {
    const runId = (getLastReport() as any)?.run_id ?? msg.run_id;
    if (runId) {
      const byRunRes = await fetch(`${RESEARCH_API_BASE}/analyze/report/pdf`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ run_id: runId }),
      });
    ...
    let res = await fetch(`${RESEARCH_API_BASE}/analyze/report/pdf`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ report: reportPayload }),
    });
    ...
    if (res.status === 422) {
      res = await fetch(`${RESEARCH_API_BASE}/analyze/report/pdf`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
export default function ResearchDemoPage() {
  console.log("API BASE:", RESEARCH_API_BASE);
  ...
  async function handleSynthGenerate(preset: string, params: Record<string, number>) {
    ...
    setSynthMode(false);
    try {
      const res = await fetch(`${RESEARCH_API_BASE}/generate/analyze`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <main className="min-h-screen bg-neutral-950 text-white flex flex-col">
      <div className={`${splitWorkspace ? "w-full px-4 py-4" : "max-w-2xl mx-auto w-full px-6 py-12"} flex flex-col flex-1`}>
        <h1 className="text-2xl font-semibold mb-1">Research Workspace Demo</h1>
        ...
        <p className="text-neutral-500 text-xs mb-6">
          Scope pipeline: DataScope → FeatureScope → Research → FactorScope → ReturnScope
        </p>
          ...
          {messages.map((msg, i) => {
            ...
            >
              <span className="text-xs text-neutral-500 block mb-1">
                {msg.role === "user" ? "You" : "Research Workspace"}
              </span>
              {/* user bubble */}
              {msg.role === "user" && (
